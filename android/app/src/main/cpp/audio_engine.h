...
public:
    ...
    float getDnnUserIntensity() const {
        // El selector forwardea setIntensity a todos; leemos de la instancia activa.
        if (denoiserSelector_.getActive() == DenoiserType::kRNNoise)
            return rnnoiseDenoiser_.getIntensity();
        if (denoiserSelector_.getActive() == DenoiserType::kDFN3)
            return dfn3Denoiser_.getIntensity();
...
private:
    ...
    /// hace en initDnnDenoiser: si libdfn3.so está disponible y los modelos
    ...
    /// GTCRN. El DenoiserSelector maneja la selección exclusiva.
    rnnoise_denoiser::RnnoiseDenoiser rnnoiseDenoiser_;

    // ─── DenoiserSelector (toggle exclusivo entre motores mono) ──────────
    /// Selector unificado que reemplaza los flags useRnnoise_/useDfn3_.
    /// Maneja crossfade 20ms entre motores al cambiar selección.
