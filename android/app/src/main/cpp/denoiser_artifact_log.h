...
/// @brief Registro de sesión de "matraca" (crackle) y calidad para los
///        sistemas de limpieza de ruido (RNNoise/DPDFNet-4).
///
...
public:
    /// Cantidad de motores de limpieza (RNNoise, DFN3, GTCRN, DPDFNet-4).
    ...
    std::string renderReport() const {
        ...
        // ─── Cada uno de los sistemas activos (skip removed slots) ────────
        for (int i = 0; i < kEngineCount; ++i) {
            // Skip removed engines (DFN3 idx=1, GTCRN idx=2)
            if (i == 1 || i == 2) continue;
...
private:
    ...
    static void appendDiagnosis(std::string& r,
        ...
        for (int i = 0; i < kEngineCount; ++i) {
            if (i == 1 || i == 2) continue;  // Skip removed engines
