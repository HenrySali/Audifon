...
public:
    /// @param impl Pointer to the DpdfnetDenoiser instance (non-owning).
    /// @param assetPath ONNX model path inside assets (default: FP32 model).
    /// @param displayName Name returned by name() for logs/UI.
    explicit DpdfnetAdapter(dpdfnet_denoiser::DpdfnetDenoiser* impl,
                            const char* assetPath = "dpdfnet/dpdfnet4.onnx",
                            const char* displayName = "DPDFNet-4")
        : impl_(impl), assetPath_(assetPath), displayName_(displayName) {}

    bool initialize(AAssetManager* mgr) override {
        return impl_->initialize(mgr, assetPath_);
    }
...
private:
    ...
    const char* assetPath_;                    ///< ONNX asset path
    const char* displayName_;                  ///< Name for logs/UI
};
