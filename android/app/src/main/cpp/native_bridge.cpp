...
Java_com_psk_hearing_1aid_1app_NativeAudioBridge_nativeStartDiagnosticRecording(
    ...
    if (!g_running.load(std::memory_order_acquire) || g_engine == nullptr) {
        LOGW("nativeStartDiagnosticRecording: engine not running");
        return JNI_FALSE;
    }

    if (filePath == nullptr) {
        LOGE("nativeStartDiagnosticRecording: null filePath");
        return JNI_FALSE;
    }

    const char* pathChars = env->GetStringUTFChars(filePath, nullptr);
    if (pathChars == nullptr) {
        LOGE("nativeStartDiagnosticRecording: failed to get string chars");
        return JNI_FALSE;
    }
    ...
    LOGI("nativeStartDiagnosticRecording: path=%s", path.c_str());
    const bool ok = g_engine->startDiagnosticRecording(path);
    return ok ? JNI_TRUE : JNI_FALSE;
}

/// Detiene la grabación de diagnóstico. Si no completó 60s, descarta el archivo.
/// @return 0=success (completado), 1=discarded (parada temprana/idle), -1=error
JNIEXPORT jint JNICALL
Java_com_psk_hearing_1aid_1app_NativeAudioBridge_nativeStopDiagnosticRecording(
    ...
    if (!g_running.load(std::memory_order_acquire) || g_engine == nullptr) {
        LOGW("nativeStopDiagnosticRecording: engine not running");
        return -1;
    }

    g_engine->stopDiagnosticRecording();

    // Check final state to determine result code
    DiagRecorderState state = g_engine->getDiagnosticRecordingState();
    switch (state) {
        case DiagRecorderState::COMPLETED:
            LOGI("nativeStopDiagnosticRecording: completed successfully");
            return 0;
        case DiagRecorderState::IDLE:
            LOGI("nativeStopDiagnosticRecording: discarded (early stop or not recording)");
            return 1;
        case DiagRecorderState::ERROR:
            LOGE("nativeStopDiagnosticRecording: error state");
            return -1;
        default:
            LOGW("nativeStopDiagnosticRecording: unexpected state %d",
...
Java_com_psk_hearing_1aid_1app_NativeAudioBridge_nativeGetDiagnosticRecordingProgress(
    ...
    return static_cast<jlong>(g_engine->getDiagnosticRecordingProgress());
}
