...
/// Cuando activo Y conversationMode=true, el denoiser se salta (~10ms menos).
extern "C" JNIEXPORT void JNICALL
Java_com_psk_hearing_1aid_1app_NativeAudioBridge_nativeSetBtBypassDenoiser(
        JNIEnv* /* env */,
        jobject /* thiz */,
        jboolean bypass) {
    if (g_engine == nullptr) return;
    g_engine->setBtBypassDenoiser(bypass == JNI_TRUE);
}

/// @return true si el bypass BT del denoiser está habilitado.
extern "C" JNIEXPORT jboolean JNICALL
Java_com_psk_hearing_1aid_1app_NativeAudioBridge_nativeGetBtBypassDenoiser(
        JNIEnv* /* env */,
        jobject /* thiz */) {
    if (g_engine == nullptr) return JNI_TRUE;
    return g_engine->getBtBypassDenoiser() ? JNI_TRUE : JNI_FALSE;
}

/// Selecciona el motor de denoising activo (toggle exclusivo).
/// @param type 0=RNNoise, 1=DFN3(removed), 2=GTCRN(removed), 3=DPDFNet, 4=DTLN
extern "C" JNIEXPORT void JNICALL
...
Java_com_psk_hearing_1aid_1app_NativeAudioBridge_nativeGetActiveDenoiser(
    ...
    // Cuando bypass está activo, process() no se llama y activeType_ no se
    // actualiza. Retornamos selected para que la UI no muestre fallback falso.
    if (g_engine->getBtBypassDenoiser()) {
